...
def filterText(update, context):
    ...
    if start != -1:
        ...
        if m != None:
            pCode = m.group(0)
        sender = update.message.first_name
        context.bot.send_message(chat_id=update.message.chat_id,reply_to_message_id=update.message.message_id, text="⭐️ **APORTE DE** ➡️ "+newReferURL(pCode))
